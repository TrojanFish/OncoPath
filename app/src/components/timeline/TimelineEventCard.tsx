...
export default function TimelineEventCard({ event, onEdit, onDelete }: TimelineEventCardProps) {
  ...
  return (
    <div className="relative pl-6 sm:pl-8 group">
      ...
      {/* Main Card Container */}
      <div className="bg-white rounded-2xl p-3.5 sm:p-6 border border-slate-200/90 shadow-sm hover:shadow-md hover:border-slate-300 transition-all space-y-3.5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              <TimelineCategoryIcon category={event.category} className="w-3.5 h-3.5" />
              <span>{categoryMeta.label}</span>
            </span>

            {/* Date Tag */}
            <span className="text-xs font-mono font-bold text-slate-900 bg-slate-100 px-2.5 py-1 rounded-xl flex items-center gap-1">
          ...
          <div className="flex items-center gap-1.5">
            {event.riskStatus === "warning" && (
              <span className="px-2.5 py-1 rounded-lg bg-rose-50 text-rose-700 border border-rose-200 text-xs font-bold flex items-center gap-1">
            ...
            <button
              onClick={handleCopy}
              className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors text-xs cursor-pointer flex items-center gap-1"
              title="复制摘要"
            >
              {copied ? (
                <>
        ...
        {event.keyFindings && Object.keys(event.keyFindings).length > 0 && (
          <div className="pt-2">
            ...
            {expanded && (
              <div className="mt-3 p-4 rounded-xl bg-white border border-slate-200/90 shadow-2xs space-y-2.5 text-xs text-slate-800 animate-fade-in">
                <div className="font-bold text-slate-900 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-1.5 flex items-center justify-between">
                  <span>结构化核验指标明细</span>
                  <span className="text-xs text-slate-400 font-mono">Structured Clinical Data</span>
                </div>
