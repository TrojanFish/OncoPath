  ...
  return (
    <>
      {userEmail ? (
        ...
        /* Guest / Unauthenticated State: Clear Distinction between Login Link & Primary Profile CTA */
        <div className="flex items-center gap-2.5 sm:gap-3">
          <button
            onClick={() => setShowAuthModal(true)}
            className="text-xs font-semibold text-slate-600 hover:text-slate-900 transition-colors px-2 py-1.5 cursor-pointer"
          >
            登录 / 注册
          </button>
          ...
          <Link
            href="/profile"
            className="btn-primary px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all shadow-sm flex items-center gap-1.5"
          >
            <span>📋</span>
            <span>建立临床档案</span>
          </Link>
