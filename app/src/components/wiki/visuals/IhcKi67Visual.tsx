...
import { Dna, FileBadge2, Activity, Info } from "lucide-react";
    ...
    <div className="bg-slate-900 rounded-2xl p-2.5 sm:p-4 text-white select-none border border-slate-800">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[11px] font-bold text-purple-400 flex items-center gap-1">
          ...
          <span>免疫组化 (IHC) 标记与 Ki-67 细胞增殖指数</span>
        </span>
        <span className="text-[10px] text-slate-400">病理报告解码</span>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {/* Left Card: IHC Identity Markers */}
        <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-2">
          <div className="text-xs font-bold text-sky-400 flex items-center gap-1.5">
            <FileBadge2 className="w-3.5 h-3.5 text-sky-400" />
            <span>肿瘤组织学特异标记 (IHC 阳性)</span>
          </div>
        ...
        {/* Right Card: Ki-67 Proliferation Index */}
        <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-2">
          <div className="text-xs font-bold text-rose-400 flex items-center gap-1.5">
            <Activity className="w-3.5 h-3.5 text-rose-400" />
            <span>细胞增殖活跃度 (Ki-67 指数)</span>
          </div>
          <div className="space-y-1.5 text-[11px]">
            <div className="bg-emerald-950/40 p-1.5 rounded-lg border border-emerald-500/30 flex justify-between items-center">
              ...
              <span className="text-[10px] text-emerald-400">极低速运转 · 极惰性</span>
            </div>
            ...
            <div className="bg-rose-950/40 p-1.5 rounded-lg border border-rose-500/30 flex justify-between items-center">
              ...
              <span className="text-[10px] text-rose-400">高速增殖 · 对化疗/靶向更敏感</span>
            </div>
          </div>
          <div className="text-[10px] text-slate-400 leading-tight">
            ※ Ki-67 是增殖期细胞百分比，<strong>绝不等于复发率</strong>，活跃细胞更容易被针对性药物杀灭。
          </div>
        </div>
      </div>
      ...
      <div className="mt-2.5 text-[11px] text-slate-300 leading-relaxed bg-slate-800/80 p-2.5 rounded-xl border border-slate-700">
        ...
        免疫组化里的 TTF-1、CK7 阳性是病理医生确认肿瘤类型（腺癌还是鳞癌）的<strong>组织学标记</strong>；Ki-67 则是反映细胞当下增殖速度的<strong>活性指标</strong>，数值高低指导用药敏感性，绝非复发转移的概率裁决。
      </div>
    </div>
  );
}
