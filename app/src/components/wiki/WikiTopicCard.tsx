  ...
  return (
    <>
      <div
        id={`topic-${topic.id}`}
        ref={cardRef}
        className={`bg-white rounded-2xl p-3.5 sm:p-6 border transition-all duration-500 flex flex-col justify-between relative ${
          isHighlighted
      ...
      >
        ...
        {isMatchedProfile && (
          <div className="absolute -top-3 right-6 bg-teal-600 text-white text-xs font-bold px-3 py-0.5 rounded-full shadow-xs flex items-center gap-1">
        ...
        {isHighlighted && !isMatchedProfile && (
          <div className="absolute -top-3 right-6 bg-blue-600 text-white text-xs font-bold px-3 py-0.5 rounded-full shadow-xs flex items-center gap-1">
          ...
          <div className="flex items-center justify-between gap-1.5 sm:gap-2 mb-3">
            <div className="flex items-center gap-1.5 flex-nowrap min-w-0">
              ...
              <button
                ...
                className="text-xs px-2.5 py-1 rounded-full border border-blue-200 bg-blue-50/90 text-blue-700 hover:bg-blue-100 transition-all cursor-pointer flex items-center gap-1 shadow-2xs whitespace-nowrap flex-shrink-0 active:scale-95"
              >
                <Image className="w-3 h-3 text-blue-600 flex-shrink-0" />
                <span className="text-xs font-bold">海报</span>
              </button>
            </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                >
                  ...
                  {showVisual ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                </button>
              </div>
              {showVisual && (
                <div className="animate-fade-in" ref={visualContainerRef}>
                  {hasBeenInView ? (
                    ...
                    <div className="h-44 rounded-2xl bg-slate-900/90 border border-slate-800 flex items-center justify-center text-slate-400 text-xs gap-2 select-none">
          ...
          {topic.keyMetric && (
            <div className="bg-blue-50/60 p-3.5 sm:p-4 rounded-2xl border border-blue-100 space-y-2 mb-4">
              <div className="flex items-center justify-between gap-2 text-xs font-bold text-blue-900">
                ...
                <span className="text-xs font-semibold text-blue-700 bg-blue-100/70 px-2 py-0.5 rounded-full border border-blue-200/60 truncate max-w-[220px]">
              ...
              <div className="text-xs text-slate-500 pt-1.5 border-t border-blue-100/80 flex items-center gap-1">
          ...
          {topic.faq && topic.faq.length > 0 && (
            <div className="space-y-2 mb-4">
              <div className="text-xs font-bold text-slate-700 flex items-center justify-between gap-1.5">
                ...
                <span className="text-xs text-slate-400">可一键加入门诊提问清单</span>
              </div>
                ...
                return (
                  <div key={idx} className="border border-slate-200 rounded-2xl overflow-hidden">
                    ...
                    >
                      ...
                      <span className="text-slate-400">{expandedFaq === idx ? <ChevronUp className="w-3.5 h-3.5"/> : <ChevronDown className="w-3.5 h-3.5"/>}</span>
                    </button>
                    {expandedFaq === idx && (
        ...
        {/* Section 5: Clinical Reassurance Box */}
          ...
          <div className="bg-gradient-to-br from-teal-50 to-emerald-50 p-4 rounded-2xl border border-teal-200/80 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                title={copied ? "已复制释疑要点" : "一键复制释疑要点"}
                aria-label="复制释疑要点"
                className={`w-8 h-8 rounded-xl border flex items-center justify-center transition-all cursor-pointer shadow-2xs flex-shrink-0 ${
                  copied
                    ? "bg-emerald-600 border-emerald-600 text-white scale-105"
                    : "bg-white/90 border-teal-300/80 text-teal-800 hover:bg-white hover:border-teal-400 active:scale-95"
                }`}
              >
                {copied ? (
