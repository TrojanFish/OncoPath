...
export function getClinicalCohortForProfile(rawProfile: any): ClinicalCohortResult {
  ...
  if (isPreOp) {
    if (stage === "0" || ctr === 0 || staging.noduleType === "pure_ggo") {
      return {
        ...
        confidenceRating: "⭐⭐⭐⭐⭐",
        confidenceLevel: "极高置信度 (1级证据)",
        source: "JCOG0804 / JCOG1211 国际前瞻多中心队列",
      ...
      return {
        ...
        confidenceRating: "⭐⭐⭐⭐⭐",
        confidenceLevel: "极高置信度 (1级证据)",
        source: "JCOG0802 / CALGB 140503 国际多中心 RCT 队列",
  ...
  if (stage === "IA1") {
    if (isStas || isVpi || isLvi || isGrade3) {
      return {
        ...
        source: "IASLC 8th Lung Cancer Staging Project & Multi-institutional Cohort",
        description: "微浸润虽局限于 IA1 期，但病理切片提示伴有微小高危征象。切缘阴性切除后整体生存率依然高达 96%，建议保持规律薄层 CT 随访。",
        isPreOp: false,
        keyFactors,
      };
    }
    return {
      ...
      confidenceRating: "⭐⭐⭐⭐⭐",
      confidenceLevel: "极高置信度 (1级证据)",
      source: "JCOG0804 / JCOG0802 / CALGB 140503 前瞻多中心队列",
      description: "极早期微浸润腺癌队列。国际指南一致公认切除后达到临床根治，5 年无复发生存率高达 98.8%，无需任何辅助化疗或靶向治疗，常规薄层 CT 随访即为最优方案。",
      isPreOp: false,
      keyFactors,
    };
  }
  ...
  if (stage === "IA2") {
    if (isStas || isVpi || isLvi || isGrade3) {
      return {
        ...
        confidenceRating: "⭐⭐⭐⭐☆",
        confidenceLevel: "高置信度 (多中心分析)",
        source: "IASLC 8th Database & Chest 2021 Meta-analysis",
    ...
    return {
      ...
      confidenceRating: "⭐⭐⭐⭐⭐",
      confidenceLevel: "极高置信度 (1级证据)",
      source: "CALGB 140503 / JCOG0802 国际顶级 RCT 队列",
  ...
  if (stage === "IA3") {
    return {
      ...
      source: "IASLC 8th/9th Staging Database & CALGB 140503",
      description: "2~3cm 原发灶完全切除队列。5 年总生存率达 93.0%，建议术后前两年每 6 个月定期复查胸部 CT 与腹部超声。",
      isPreOp: false,
      keyFactors,
    };
  }
  ...
  if (stage === "IB") {
    return {
      ...
      confidenceRating: "⭐⭐⭐⭐⭐",
      confidenceLevel: "极高置信度 (1级证据)",
      source: "ADAURA (NEJM / Lancet) & LACE Collaborative Meta-analysis",
      description: "IB 期病灶完全切除队列。若肿瘤实性径接近 4cm 或伴胸膜侵犯且有 EGFR 敏感突变，ADAURA 试验证实三代靶向药辅助治疗可将无病生存率大幅提升至 90% 以上。",
      isPreOp: false,
      keyFactors,
    };
  }
  ...
  if (stage === "IIA") {
    return {
      ...
      source: "ADAURA & ANITA / LACE Collaborative Group",
  ...
  if (stage === "IIB") {
    return {
      ...
      confidenceRating: "⭐⭐⭐⭐☆",
      confidenceLevel: "高置信度 (1级证据)",
      source: "IMpower010 / ADAURA / IASLC 8th Staging Project",
  ...
  if (stage === "IIIA") {
    return {
      ...
      confidenceRating: "⭐⭐⭐⭐⭐",
      confidenceLevel: "极高置信度 (1级证据)",
      source: "ADAURA (NEJM 2023) / ALINA (NEJM 2024) / KEYNOTE-091",
      description: "IIIA 期伴纵隔 N2 累及队列。在当今第三代靶向药（奥希替尼等）和免疫治疗时代，3 年无病生存率已突破 75%~85%，长期生存格局迎来跨时代突破。",
      isPreOp: false,
      keyFactors,
    };
  }
  ...
  if (stage === "IIIB" || stage === "IIIC") {
    return {
      ...
      confidenceRating: "⭐⭐⭐⭐☆",
      confidenceLevel: "高置信度 (1级证据)",
      source: "PACIFIC Trial (NEJM / Lancet Oncology 5-Year Update)",
      description: "PACIFIC 模式（同步放化疗序贯度伐利尤单抗免疫巩固）5 年总生存率达 42.9%~45%，建立了局部进展期非小细胞肺癌长生存新标杆。",
      isPreOp: false,
      keyFactors,
    };
  }
