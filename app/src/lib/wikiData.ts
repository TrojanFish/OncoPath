...
export const WIKI_TOPICS: WikiTopic[] = [
  ...
  },
  {
    ...
    keyMetric: {
      label: "IASLC / AJCC 第 9 版分期规则",
      value: "T1 (≤3cm) 伴 PL1/PL2 自动升期为 T2a",
      source: "Travis et al., JTO / IASLC 9th Staging Cards (2024)",
    },
    faq: [
      {
  ...
  },
  {
    ...
    clinicalTruth: "PL3 是指肿瘤直接穿透胸膜腔侵犯至壁层胸膜或胸壁组织（如肋骨、肋间肌）。依据 IASLC / AJCC 第 9 版最新分期标准，无论肿瘤原本多小，一旦出现 PL3，T 分期直接定为 T3 期（若 N0 为 IIB 期；伴 N1 为 IIIA 期；伴 N2a 单站纵隔为 IIIA 期，伴 N2b 多站纵隔为 IIIB 期）。需要注意：第 9 版定义的 T3 还包括：侵犯心包、膈神经、奇静脉、胸神经根 (T1/T2) 或星状神经节、以及同叶内副结节（注：侵犯膈肌与同侧不同叶结节属于 T4 分期）。外科标准治疗原则为整块切除（En-bloc 切除），保证切缘病理阴性（R0），术后依据病理及驱动基因状态常规行辅助靶向/化疗/放疗。",
    tactics: [
      ...
      "对于 T3N2M0 患者，IASLC 第 9 版将单站 N2a 优化为 IIIA 期，多站 N2b 划为 IIIB 期，NCCN 推荐由 MDT 讨论综合放化疗或围手术期靶向/免疫方案",
    ...
    keyMetric: {
      label: "IASLC / AJCC 第 9 版 T3分期标准",
      value: "壁层胸膜/胸壁受累直接定为 T3",
      source: "IASLC Staging Cards 9th Edition (2024)",
    },
    faq: [
      {
  ...
  // ==================== 2. 肺结节消恐与随访 (按风险高低排序) ====================
  {
    ...
    clinicalTruth: "磨玻璃结节（GGO）是指薄层 CT 上呈现密度轻度增高、但内部仍能隐约看清支气管和血管纹理的结节。CTR（Consolidation-to-Tumor Ratio，实性成分比 = CT 纵隔窗实性成分最大径 ÷ 肺窗病灶整体最大径）是国际公认判断浸润性与制定手术策略的金标准。特别强调：根据国际 IASLC / AJCC 第 9 版现行分期标准，对于部分实性结节（mGGO），T 分期严格仅由'实性成分最大径'决定，而非病灶总径（例如总径 2.2cm 但实性成分仅 0.8cm，精准分期为 T1a，而非 T1c）！当 CTR ≤ 0.5 时，极大概率为原位癌（AIS）或微浸润腺癌（MIA），5 年无复发生存率接近 100%。",
    tactics: [
    ...
    keyMetric: {
      label: "IASLC / AJCC 第 9 版 T分期规则",
      value: "仅以 CT 实性成分最大径定 T 期 (CTR ≤ 0.5 5年RFS 99.7%)",
      source: "IASLC 9th Staging Cards (2024) / JCOG0804 Trial",
    },
    faq: [
      {
  ...
  },
  {
    ...
    clinicalTruth: "胸部 CT 具有窗宽 (WW) 与窗位 (WL) 的物理学区分，是精准定级与手术决策的关键基石：\n• 肺窗 (WL: -600, WW: 1500)：专门用于观察含气充分的细微肺实质与磨玻璃光晕（GGO），可清晰显示伏壁贴壁生长的惰性细胞与微小血管走行；\n• 纵隔窗 (WL: 40, WW: 400)：专门过滤低密度含气成分，仅显示真正具有侵袭破坏力的实性软组织成分与淋巴结；\n• IASLC / AJCC 第 9 版权威金标准：肺癌 T 分期浸润大小严格以纵隔窗实性成分或病理实性浸润径为准，严禁把外围惰性磨玻璃粗暴计入分期大小；\n• 电子卡尺与 CTR (实性比)：实性径 / 全径比值决定病理亚型倾向与 JCOG0802/0804 亚肺叶切除（肺段/楔形）适应证。",
    tactics: [
  ...
  {
    id: "drug-drug-interactions-ddi",
    category: "recovery",
    subcategory: "临床药学与药物警戒",
    title: "靶向药与慢病常见用药相互作用 (DDI) 排查指南：胃药、降压降脂、抗凝与日常西柚禁忌",
    subtitle: "Drug-Drug Interactions (DDI) & Pharmacovigilance: Targeted Therapies vs Chronic Medications",
    icon: "pill",
    riskLevel: "moderate",
    priorityOrder: 9,
    visualComponent: "DdiCheckerVisual",
    metaphor: "靶向药就像进入人体作战的特种侦察兵，而日常吃的胃药、降压药或某些水果（如西柚）可能会在肝脏和胃肠道里‘堵塞通道’或‘误伤友军’——学会科学错峰服药，才能让特种兵发挥 100% 战斗力同时保全身体各项机能！",
    clinicalTruth: "肺腺癌靶向治疗常需持续数月至数年，患者常合并高血压、冠心病、反流性食管炎等基础慢病。临床药代动力学 (PK) 与药物相互作用 (DDI) 存在三大核心规律：\n• ① 绝对严重禁忌：严禁食用西柚/葡萄柚（破坏肠道 CYP3A4 导致靶向药浓度暴增中毒）、严禁联用利福平/卡马西平/圣约翰草（强效诱导 CYP3A4 导致靶向药浓度骤降 80% 完全失效）；\n• ② 消化道抑酸药错峰：一代吉非替尼/厄洛替尼溶解度高度依赖胃酸，长期合用奥美拉唑等 PPI 会使药效减半，建议优先换用法莫替丁，且服用抗酸剂（达喜/硫糖铝）必须【严格错峰 2 小时以上】；\n• ③ 心血管与降脂药管理：氨氯地平、缬沙坦等常规降压药相容性良好；阿托伐他汀合用需监测肌酶，可优先换用低代谢竞争的瑞舒伐他汀。",
    tactics: [
      "使用下方交互式 DDI 排查工具，一键勾选您当前服用的所有日常慢病药物，自动生成红/黄/绿风险报告与 24 小时服药时钟",
      "服药期间严禁食用西柚（红心柚）、西柚果汁、含圣约翰草的抗抑郁安神保健品",
      "如有胃痛反酸需吃胃药（达喜、奥美拉唑等），必须与靶向药错开至少 2~4 小时分别服用，切勿同杯水吞服",
      "每次去其他科室（心内科、消化科、神经科）就诊时，务必主动告知医生‘我正在吃抗癌靶向药’",
    ],
    reassurance: "绝大多数高血压、糖尿病和心血管慢病药物与靶向药都能和平共处！只要掌握‘错峰服用、避开绝对禁忌、遵医嘱定期复查肝功能’的原则，您完全无需为多药联用感到恐慌！",
    keyMetric: {
      label: "科学错峰服药与防范 DDI 效益",
      value: "靶向药血药浓度达标率与安全性提升 40%+",
      source: "Clinical Pharmacokinetics & NCCN Non-Small Cell Lung Cancer Guidelines 2024",
    },
    faq: [
      {
        question: "为什么吃奥希替尼等靶向药期间，医生千叮咛万嘱咐绝对不能吃西柚（葡萄柚）？",
        answer: "因为西柚中含有高浓度的呋喃香豆素，它会强效、不可逆地破坏肠道中的 CYP3A4 代谢酶。原本由该酶分解的靶向药会大量涌入血液，使血药浓度瞬间暴增 3~5 倍，极易引发严重皮疹、剧烈腹泻、转氨酶暴发性升高甚至恶性心律失常，因此属于绝对禁忌！"
      },
      {
        question: "我有反流性食管炎，天天要吃奥美拉唑，和靶向药该怎么协调？",
        answer: "奥美拉唑会强力抑制胃酸，使胃内 pH 上升，对一代吉非替尼/厄洛替尼影响较大（三代奥希替尼受影响相对较小）。建议：1) 咨询消化科医生是否能换成法莫替丁；2) 如果必须吃胃药，早晨 8 点吃靶向药，下午 14:00 或睡前再吃胃药，错开 2~4 小时，避免同服。"
      },
      {
        question: "降压药（络活喜、代文）和靶向药能一起吃吗？",
        answer: "可以！氨氯地平（络活喜）、硝苯地平控释片（拜新同）、缬沙坦（代文）与常见靶向药在临床常规剂量下相容性良好，不会影响靶向药的抗癌效果。按心内科医嘱规律每天定时服用降压药即可。"
      }
    ],
    searchKeywords: ["DDI", "相互作用", "靶向药禁忌", "西柚", "葡萄柚", "奥美拉唑", "达喜", "胃药", "降压药", "他汀", "阿司匹林", "yaowuxianghuzuoyong", "泰瑞沙用药禁忌", "安圣莎注意事项"],
  },
];
