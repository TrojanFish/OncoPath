...
export async function POST(request: Request) {
  try {
    ...
    const rateLimit = checkRateLimit(`evidence_europe_pmc_${clientIp}`, { intervalMs: 60 * 1000, maxRequests: 15 });
    ...
    if (!query) {
      query = '("stage IA" OR "T1N0") AND "lung cancer"';
    }
    ...
    for (const item of rawList) {
      ...
      if (!existing) {
        await prisma.ingestedStudy.create({
          data: {
            title,
            journal: item.journalTitle || item.journalInfo?.journal?.title || 'Europe PMC Journal',
            ...
            evidenceLevel: 4,
            summary: item.abstractText?.replace(/<[^>]*>?/gm, '') || '已从 Europe PMC 获取文献摘要。',
            conclusion: item.abstractText ? item.abstractText.replace(/<[^>]*>?/gm, '').slice(0, 300) + '...' : '等待临床循证提炼',
            keywords: 'Europe PMC, NSCLC, Evidence',
    ...
    return NextResponse.json({
      success: true,
      message: `Europe PMC 抓取完成，成功同步入库 ${insertedCount} 篇文献。`,
      insertedCount,
      totalFetched: rawList.length,
    });
  } catch (error: any) {
    console.error('Error in /api/evidence/fetch-europe-pmc:', error);
    return NextResponse.json({ success: false, error: error.message || 'Europe PMC 抓取失败' }, { status: 500 });
  }
}
