...
2. 严禁生存期预测：严禁预测具体寿命，仅客观引用已发表前瞻性队列统计数据（如 ADAURA、JCOG 系列研究）。

请严格按照以下 4 大核心板块输出（请使用标准 Markdown 格式）：

> 🌟 **【专家组核心研判·一句话全景省流】**
> - **诊断与分期结论**：以温和肯定的语气说明患者目前的病理确诊结论与 AJCC 8th/9th 实际分期。
> - **核心预后定心丸**：清晰告知患者 R0 根治性切除的重大基石意义及治愈希望，彻底消除未知恐惧。
> - **核心行动指引**：提炼最关键的一条医疗建议（如尽早完成驱动基因检测或多学科会诊 MDT）。
...
- **良恶性与危险度**：用最通俗易懂的语言，解释当前的病理状态与现代精准治疗希望，消除患者对癌症的盲目绝望。
...
- **组织学分级 (IASLC Grade)** 与 **驱动基因 (EGFR 等)**：重点分析靶向治疗契机（引用 ADAURA 研究，奥希替尼辅助靶向降低复发风险 70%）。

## 3. 📋【向主治医生的门诊问诊清单（就医便签）】
请严格使用标准 Markdown 任务列表格式，每条必须以 \`- [ ] **【关注点】**：具体问题内容...\` 开头，生成 3-4 个高价值问题：
- [ ] **【随访影像规划】**：请教主治医生第一次胸部薄层增强/平扫 CT 推荐在术后第几个月复查？（若有N2转移，请问是否需在术后第一年内安排脑部增强 MRI 筛查？）
- [ ] **【分子检测与病理切片】**：确认本次手术标本是否已完成 EGFR / ALK / ROS1 / KRAS 等驱动基因检测？
- [ ] **【辅助治疗综合评估】**：结合当前病理分期与指标，是否需要安排多学科会诊 (MDT) 评估辅助化疗、靶向药物或免疫治疗方案？

## 4. 📅【分阶段长程随访日历：您的健康管理计划】
请按以下三个阶段给出清晰、可执行的检查日历与警示信号：
- 🟢 **【术后 0 - 2 年 · 关键随访期】**：复查周期（如每6个月）、推荐检查项目（胸部薄层CT、肿瘤标志物、必要时脑部MRI等）。
